...
namespace PW_LAB_3
{
    public partial class Client : Form
    {
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            double[,] m1_1 = new double[256, 256];
            double[,] m1_2 = new double[256, 256];
            double[,] m1_3 = new double[256, 256];
            double[,] m1_4 = new double[256, 256];

            double[,] m2_1 = new double[256, 256];
            double[,] m2_2 = new double[256, 256];
            double[,] m2_3 = new double[256, 256];
            double[,] m2_4 = new double[256, 256];
            ...
            for (int i = 0; i < 1024; i++)
            {
                for (int j = 0; j < 1024; j++)
                {
                    ...
                    else if (i > 256 && i <= 512 && j > 256 && j <= 512)
                    {
                        m1_2[i - 256, j - 256] = matrix_1.matrix[i, j];
                        m2_2[i - 256, j - 256] = matrix_1.matrix[i, j];
                    }
                    // Trzecia ćwiartka macierzy
                    else if (i >= 512 && i <= 768 && j >= 512 && j <= 768)
                    {
                        m1_3[i - 512, j - 512] = matrix_1.matrix[i, j];
                        m2_3[i - 512, j - 512] = matrix_1.matrix[i, j];
                    }
                    // Czwarta ćwiartka macierzy
                    else 
                    {
                        m1_4[i - 768, j - 768] = matrix_1.matrix[i, j];
                        m2_4[i - 768, j - 768] = matrix_1.matrix[i, j];
                    }
                }
            }
